...
if execution_premission "Create blank local overlay? "; then
	echo
	read -p "Enter name of the local overlay (default: "${local_overlay_name}") " local_overlay_name
	mkdir -p ${work_dir}/var/overlay/${local_overlay_name}
	git clone https://github.com/funtoo/skeleton-overlay.git ${work_dir}/var/overlay
	mv ${work_dir}/var/overlay/skeleton-overlay ${work_dir}/var/overlay/${local_overlay_name}
	echo ${local_overlay_name} > ${work_dir}/var/overlay/${local_overlay_name}/profiles/repo_name
	echo "masters = core-kit" >> ${work_dir}/var/overlay/${local_overlay_name}/metadata/layout.conf
	echo -e "[DEFAULT]/nmain-repo = core-kit/n["${local_overlay_name}"]\
